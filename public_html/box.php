...
function &create_pdf_box(&$root, &$pipeline) {
  ...
  if (is_object($root)) {
    ...
    case XML_TEXT_NODE:
      ...
      return $box;
    default:
      throw new \Exception("unsupported node type:".$root->node_type());
    }
  } else {
...
function &create_node_box(&$root, &$pipeline) {
  ...
  default:
     ...
     */
    log_error("Unsupported 'display' value: ".$css_state->get_property(CSS_DISPLAY));
    $box = BlockBox::create($root, $pipeline);
    break;
  }
  ...
  if ($need_block_wrapper) {
    ...
    $box->setCSSProperty(CSS_MARGIN, $handler->default_value_m());
    ...
    $box->setCSSProperty(CSS_PADDING, $handler->default_value_m());

    $handler = CSS::get_handler(CSS_BACKGROUND);
    $box->setCSSProperty(CSS_BACKGROUND, $handler->default_value_m());
